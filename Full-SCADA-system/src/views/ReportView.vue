...
// ─── Config ───
const API_SCADA_HISTORY = `${FACTORY_URL}/scada/history`
...
async function fetchData() {
  ...
  errorMessage.value = ''
  try {
    const res = await fetch(`${API_SCADA_HISTORY}?date=${selectedDate.value}`)
...
async function fetchRange(start, end) {
  ...
  const results = await Promise.all(dates.map(async (date) => {
    try {
      const res = await fetch(`${API_SCADA_HISTORY}?date=${date}`)
      if (!res.ok) return []
      const json = await res.json()
